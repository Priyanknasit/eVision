...
public class MultiBoxTracker {
  ...
  public static float obh1,obh2,obh1w,obh2w;
  public static float imgh,imgw;
  ...
  public synchronized void draw(final Canvas canvas) {
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if (recognition.title.equals("person")) {
        obh1 = trackedPos.height();
        obh1w = trackedPos.centerX();
        if (obh1w<(imgw/2)) {
          labelString = !TextUtils.isEmpty(recognition.title)
                  ? String.format("%s on %s ",recognition.title,"Left")
                  : String.format("%.2f", recognition.detectionConfidence);

        }
        else {
          labelString = !TextUtils.isEmpty(recognition.title)
                  ? String.format("%s on %s ",recognition.title,"Right")
                  : String.format("%.2f", recognition.detectionConfidence);
        }
      ...
      if (recognition.title.equals("chair")) {
        obh2 = trackedPos.height();
        obh2w = trackedPos.centerX();
        if (obh2w<(imgw/2)) {
          labelString = !TextUtils.isEmpty(recognition.title)
                  ? String.format("%s on %s ",recognition.title,"Left")
                  : String.format("%.2f", recognition.detectionConfidence);
      ...
      /*final String labelString =
          !TextUtils.isEmpty(recognition.title)
              ? String.format("%f %s %.2f", obh1 , recognition.title, recognition.detectionConfidence)
              : String.format("%.2f", recognition.detectionConfidence);
*/
