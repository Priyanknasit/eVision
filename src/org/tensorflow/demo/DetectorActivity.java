...
import static org.tensorflow.demo.tracking.MultiBoxTracker.imgh;
import static org.tensorflow.demo.tracking.MultiBoxTracker.imgw;
import static org.tensorflow.demo.tracking.MultiBoxTracker.obh1;
import static org.tensorflow.demo.tracking.MultiBoxTracker.obh1w;
import static org.tensorflow.demo.tracking.MultiBoxTracker.obh2;
import static org.tensorflow.demo.tracking.MultiBoxTracker.obh2w;
                      ...
                      if (mappedRecognitions.size()==0 && currentSpeechTurn == TALK_SPEECH_TURN){
                        ...
                        for (Occurences spokenOccurence : mappedOccurences){
                          ...
                          //speakOut(and + spokenOccurence.getObjectOccurence()+ " " +spokenOccurence.getObjectTitle() + plural_s);
                            if (spokenOccurence.getObjectTitle().equals("person")) {
                              float d = (26 * 1700 * imgh) / ((obh1) * 1700);
                              int steps = (int) ((d / 10) / 1.5);
                              if (steps < 20) {
                                if ((obh1w) < (imgw/2)) {
                                  speakOut(spokenOccurence.getObjectOccurence() + " " + spokenOccurence.getObjectTitle() + " is on Left at " + String.valueOf(steps) + " steps.");
                                  sleep(500);
                                }
                                else {
                                  speakOut(spokenOccurence.getObjectOccurence() + " " + spokenOccurence.getObjectTitle() + " is on Right at " + String.valueOf(steps) + " steps.");
                                  sleep(500);
                                }
                              }
                            }

                          if (spokenOccurence.getObjectTitle().equals("chair")) {
                            float d = (26 * 800 * imgh) / ((obh2) * 1700);
                            int steps = (int) ((d / 10) / 1.5);
                            //speakOut(and+String.valueOf(obh2));
                            if (steps < 20) {
                              if ((obh2w) < (imgw/2)) {
                              speakOut(spokenOccurence.getObjectOccurence() + " " + spokenOccurence.getObjectTitle() + " is on Left at " + String.valueOf(steps) + " steps.");
                              sleep(500);
